...
@Client.on_message(command(["eval","غير", f"eval{bname}"]) & ~filters.edited)
@sudo_users_only
async def executor(client, message):
    if len(message.command) < 2:
        return await edit_or_reply(message, text="» اعطني كوماند لتغيره")
    try:
...
@Client.on_message(command(["leavebot","وت_غادر", f"leavebot{bname}"]) & ~filters.edited)
@sudo_users_only
async def bot_leave_group(_, message):
    ...
    try:
        ...
        await message.reply_text(f"❌ فشلت العمليه\n\nreason: `{e}`")
        print(e)
        return
    await message.reply_text(f"✅ تم مغادره البوت بنجاح:\n\n💭 » `{chat}`")
